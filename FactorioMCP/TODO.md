...
- [ ] **Improve TraceItemFlow to produce useful end-to-end factory analysis** — ✅ **COMPLETED** — **(1)** Belt collapsing: ✅ Implemented — simple belt chains are collapsed into single "belt_segment" nodes with `belt_length`, `end_x`, `end_y` metadata; belt segments cost 0 depth hops. **(2)** Inserter drop-target radius: ✅ Fixed — increased from 0.5 to 1.5 in both `TraceItemFlowAsync` and `GetFlowGraphAsync`. **(3)** Machine pass-through: ✅ Implemented — inserter detection uses `pickup_position` + bounding box fallback when `pickup_target` is nil (idle machines); machines include recipe info in trace nodes. **(4)** Underground belt traversal: ✅ Implemented — `belt_to_ground_type` + `neighbours` used to follow input→output pairs; underground traversal costs 0 depth like belt segments.
